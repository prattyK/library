...
public class DistanceNode {
	 ...
	 * https://leetcode.com/problems/all-nodes-distance-k-in-binary-tree/ This code
	 * passes all Leetcode test cases as of Feb. 12th 2019 Runtime: 3 ms, faster
	 * than 64.42% of Java online submissions for All Nodes Distance K in Binary
	 * Tree. (can't get much faster than that. Asymptotically this is identical to
	 * anything even remotely faster) The video to explain this code is here:
	...
	public List<Integer> distanceK(TreeNode treeRoot, TreeNode startNode, int targetDistance) {
		...
		/*
		 * Create the queue that we will use for the breadth first search. Add the start
		 * node to the queue
		 */
		 ...
		 * could only go down the tree. Therefore, we need a hashtable to keep track of
		 * nodes we have visited so that we do not go back and revisit what has already
		...
		while (!queue.isEmpty()) {
			...
			for (int i = 0; i < layerSize; i++) {
				...
				/*
				 * Has left been touched before? No? 1.) Add it to the seen hashtable 2.) Add it
				 * to the search queue
				 */
				...
				/*
				 * Has right been touched before? No? 1.) Add it to the seen hashtable 2.) Add
				 * it to the search queue
				 */
				...
				/*
				 * Has this node's parent been touched before? No? 1.) Add it to the seen
				 * hashtable 2.) Add it to the search queue
				 */
